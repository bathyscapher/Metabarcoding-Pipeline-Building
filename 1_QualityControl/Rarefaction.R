...
rarefied16S.merged$vineyard <- substr(rarefied16S.merged$Sample, 2, 3)


################################################################################
### 18S
rarefied18S <- read.table("euk/filtered/smpe.filter.cat.unique.good.filter.unique.precluster.pick.pick.opti_mcc.groups.rarefaction",
                          sep = "\t", header = TRUE, check.names = FALSE)
...
ggplot(data = rarefied16S.merged) +
  ...
  geom_line(aes(x = numsampled, y = hci, group = Sample), color = "gray85",
            size = 0.3) +
  geom_line(aes(x = numsampled, y = OTUs, color = vineyard, group = Sample),
            size = 0.5) +
  facet_grid( ~ Primer) +
  ...
  xlab("Reads") +
  ylab("OTUs")
ggsave("Rarefaction.pdf", width = 11.69, height = 7)
