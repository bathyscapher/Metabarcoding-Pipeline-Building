...
#  The core microbiome - common core ####
...
# prepare datasets for VennDiagramm
#Bare ground
core.BG <- subset_samples(wine.core, treatment == "Bare ground") # select bare ground samples
any(taxa_sums(core.BG) == 0) # OTUs with abundance 0
core.BG = prune_taxa(taxa_sums(core.BG) > 0, core.BG) # only keep OTUs lager >0
core.BG <- row.names(tax_table(core.BG))

# Alternating cover 
core.AC <- subset_samples(wine.core, treatment == "Alternating cover") # select bare ground samples
any(taxa_sums(core.AC) == 0) # OTUs with abundance 0
core.AC = prune_taxa(taxa_sums(core.AC) > 0, core.AC) # only keep OTUs lager >0
core.AC <- row.names(tax_table(core.AC))

# Complete cover 
core.CC <- subset_samples(wine.core, treatment == "Complete cover") # select bare ground samples
any(taxa_sums(core.CC) == 0) # OTUs with abundance 0
